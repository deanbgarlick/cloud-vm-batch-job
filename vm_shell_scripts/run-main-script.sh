...
echo "Starting VM runner setup at $(date)"
...
EXAMPLE_CUSTOM_PARAM=$(curl -s "http://metadata.google.internal/computeMetadata/v1/instance/attributes/example-custom-param" -H "Metadata-Flavor: Google" 2>/dev/null || echo "dispatcher")

echo "Configuration:"
echo "  Repository: $REPO_URL"
echo "  Branch: $BRANCH"
echo "  Auto-shutdown: $AUTO_SHUTDOWN"
echo "  Example-Custom-Param: $EXAMPLE_CUSTOM_PARAM"
...
cat > .env << 'EOF'
# GCP Configuration
GCP_ARTIFACT_BUCKET=vm-orchestration-vm-runner-artifact-bucket-1
EOF
...
LOG_BUCKET="vm-orchestration-vm-runner-logs-bucket-1"
GCP_ARTIFACT_BUCKET="vm-orchestration-vm-runner-artifact-bucket-1"
